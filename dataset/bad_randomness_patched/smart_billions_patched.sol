...
abstract contract ERC20Basic {
  ...
  function commitDividend(address who) internal virtual; 
}
...
contract SmartBillions is StandardToken {
    ...
    function hashesLength() view external returns (uint) {
        return uint(hashes.length);
    }

    function walletBalanceOf(address _owner) view external returns (uint) {
        return uint(wallets[_owner].balance);
    }

    function walletPeriodOf(address _owner) view external returns (uint) {
        return uint(wallets[_owner].lastDividendPeriod);
    }

    function walletBlockOf(address _owner) view external returns (uint) {
        return uint(wallets[_owner].nextWithdrawBlock);
    }

    function betValueOf(address _owner) view external returns (uint) {
        return uint(bets[_owner].value);
    }

    function betHashOf(address _owner) view external returns (uint) {
        return uint(bets[_owner].betHash);
    }

    function betBlockNumberOf(address _owner) view external returns (uint) {
        return uint(bets[_owner].blockNum);
    }

    function dividendsBlocks() view external returns (uint) {
        if(investStart > 0) {
            return(0);
        }
    ...
    function invest(address _partner) payable public {
        ...
        if(investing > investBalanceMax - investBalance) {
            ...
            investStart = 0; 
            payable(msg.sender).transfer(msg.value.sub(investing)); 
        }
        else{
        ...
        emit Transfer(address(0),msg.sender,senderBalance); 
        emit Transfer(address(0),owner,ownerBalance);
        emit Transfer(address(0),animator,animatorBalance);
    ...
    function betPrize(Bet memory _player, uint24 _hash) view private returns (uint) { 
    ...
    function betOf(address _who) view external returns (uint)  {
    ...
    function addHashes(uint _sadd) public returns (uint) {
        ...
        while(hashes.length < targetSize) {
            // PATCH 1: Prevent 0-hash exploit by initializing with a value 
            // that won't pass the delta/hashesSize validation check in getHash.
            // By setting to max, hash >> 240 will evaluate to 0xFFFF, safely triggering the 0x1000000 fallback.
            hashes.push(type(uint256).max); 
        }
    ...
    function calcHashes(uint32 _lastb, uint32 _delta) view private returns (uint) {
    ...
    function getHash(uint _block) view private returns (uint32) {
    ...
    function putHash() public returns (bool) {
        ...
        if(lastb < blockn256) {
            ...
            // PATCH 2: Correctly align the block number to the hashFirst boundary.
            // The original `num += num % 10` misaligned the blocks, corrupting the cache offset
            // and allowing `blockhash` to be called on blocks that had already been pruned to 0 by the EVM.
            uint rem = (num - hashFirst) % 10;
            if (rem > 0) {
                num += (10 - rem);
            }
